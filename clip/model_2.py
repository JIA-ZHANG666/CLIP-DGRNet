...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        #attn_output, attn_weight = self.attn(x, x, x, need_weights=True, attn_mask=self.attn_mask)#[0]
        #attn_output2 = self.attn(x, x, x, need_weights=True, attn_mask=self.attn_mask)[0]
        return self.attn(x, x, x, need_weights=True, attn_mask=self.attn_mask)#[0]
        #return attn_output2, attn_weight

    def forward(self, x: torch.Tensor):
        #print("###########x:",x.shape)
        attn_output, attn_weight = self.attention(self.ln_1(x))#(L,N,E)  (N,L,L)
        #print("########attn_output:",attn_output.shape)
        #print("########attn_output[0]:",attn_output[0].shape)
        #attn_output = self.attention(self.ln_1(x))#(L,N,E)  (N,L,L)
        x = x + attn_output
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        attn_weights = []
        x, attn_weight = self.resblocks(x)
        #x1 = self.resblocks(x)
        """with torch.no_grad():
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, H, W):
        ...
        #print("###########x4:",x.shape)#x4: torch.Size([50, 1, 768])
        #x, attn_weight = self.transformer(x)
        x, attn_weight = self.transformer(x)
        print("###########x2:",x.shape)#x4: torch.Size([50, 1, 768])
...
def build_model(state_dict: dict):
    """
    根据 state_dict 推断模型类型（ResNet or ViT），并构建 CLIP 模型。
    如果是 ViT-B/32，vision_patch_size=32。
    """
    ...
    if vit:
        ...
        # 这里自动推断 patch_size
        vision_patch_size = state_dict["visual.conv1.weight"].shape[-1]  # 应为 32
        grid_size = round((state_dict["visual.positional_embedding"].shape[0] - 1) ** 0.5)
        image_resolution = vision_patch_size * grid_size
    else:
        counts: list = [len(set(k.split(".")[2] for k in state_dict if k.startswith(f"visual.layer{b}")))
                        for b in [1, 2, 3, 4]]
